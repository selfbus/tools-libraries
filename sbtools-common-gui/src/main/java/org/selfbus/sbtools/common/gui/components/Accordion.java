...
/**
 * An Accordion provides a component that is similar to a JTabbedPane, but instead of maintaining
 * tabs, it uses accordion-style bars to control the visible component
 */
public class Accordion extends JPanel implements ActionListener
{
   private static final long serialVersionUID = -9109114210194756003L;
   ...
   /**
    * Creates a new Accordion; after which you should make repeated calls to addBar() for each bar.
    */
   public Accordion()
   {
   ...
   {
      int currentBar = 0;
      for (Iterator<String> it = bars.keySet().iterator(); it.hasNext(); ++currentBar)
      {
         String barName = (String) it.next();
