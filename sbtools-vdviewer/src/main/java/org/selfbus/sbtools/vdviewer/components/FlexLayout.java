...
package org.selfbus.sbtools.vdviewer.components;
...
/**
 * This class implements a combination of the GridBag and Border layouts. The container is
 * subdivided into rows and columns (basically a grid). Each cell can be empty or can contain a
 * component and a component can occupy one or more cells. If a component is smaller than its cell,
 * it can be aligned both horizontally and vertically. The width of a column is the width of its
 * largest component. The height of a row is the height of its tallest component. Furthermore, a row
 * or a column can be expandable, that is its width/height changes depending on the container's
 * size.<BR>
 * <BR>
 * <B>General design issues:</B><BR>
 * First, decide the grid size. Consider that some components (like panels) usually spread over more
 * cells so it is a good idea to make the grid a little bigger.<BR>
 * <BR>
 * Second, decide which row/column expand (this is done by the setColProp/setRowProp methods).<BR>
 * <BR>
 * Third, add components to container (layout) giving proper constraints.<BR>
 * <BR>
 * <B>Example:</B><BR>
 * 
 * <PRE>
 ...
 * <BR>
 * <I>Description:</I><BR>
 * line 1: creates a FlexLayout of 2 columns and 3 rows. The two 4 represent the gap between each
 * column and each row. To modify this gap use setXgap and setYgap.<BR>
 * line 2 & 3: column 1 and row 2 are set to expandable (the default is USEPREFERRED) remember that
 * row and column id starts from index 0.<BR>
 * line 4: the layout is added to the container.<BR>
 * line 5 & 6: add two labels in position (0,0) and (0,1) line 7 & 8: add two textfields in position
 * (1,0) and (1,1). The x means that the width will be expanded to occupy all column size. Because
 * column 1 is set to expandable, when the container is resized the textfields are resized too.<BR>
 * line 9: adds a panel in position (0,2). Then panel is expanded both horizontally and vertically
 * and spreads over 2 cells in x direction.<BR>
 * <BR>
 * The complete command string is:<BR>
 * 
 * <PRE>
 ...
 * 
 * <B>Notes:</B><BR>
 * - if a component occupies more than 1 cell in x direction, its preferred width is NOT taken into
 * account (same for y).<BR>
 * - if a column / row is empty, its size is set to a default value. This value can be changed using
 * setXNullgap/setYNullgap methods. <BR>
 * <BR>
 * Have fun!!!
 * 
...
public class FlexLayout implements LayoutManager
{
   ...
   public void addLayoutComponent(String name, Component comp)
   {
      ...
      if (strTk.hasMoreTokens())
      {
         ...
         if (align.equals("l"))
            xalign = FlexCell.LEFT;
         if (align.equals("c"))
            xalign = FlexCell.CENTERX;
         if (align.equals("r"))
            xalign = FlexCell.RIGHT;
         if (align.equals("x"))
            xalign = FlexCell.EXPANDX;
      }
      if (strTk.hasMoreTokens())
      {
         String align = strTk.nextToken().toLowerCase();
         ...
         if (align.equals("t"))
            yalign = FlexCell.TOP;
         if (align.equals("c"))
            yalign = FlexCell.CENTERY;
         if (align.equals("b"))
            yalign = FlexCell.BOTTOM;
         if (align.equals("x"))
            yalign = FlexCell.EXPANDY;
      }
      if (strTk.hasMoreTokens())
   ...
   public Dimension preferredLayoutSize(Container parent)
   {
      synchronized (parent.getTreeLock())
      {
         ...
         return new Dimension(insets.left + insets.right + maxW + (width - 1) * xgap, insets.top + insets.bottom + maxH
            + (height - 1) * ygap);
      }
   }
      ...
      synchronized (parent.getTreeLock())
      {
         ...
         for (int y = 0; y < height; y++)
         {
            ...
            for (int x = 0; x < width; x++)
            {
               ...
               if (cell != null)
               {
                  for (xc = x; xc < x + cell.xext; xc++)
                     cellW += xpref[xc];
                  for (yc = y; yc < y + cell.yext; yc++)
                     cellH += ypref[yc];
                  ...
                  cell.component
                     .setBounds(compX, compY, compW + (cell.xext - 1) * xgap, compH + (cell.yext - 1) * ygap);
               }
